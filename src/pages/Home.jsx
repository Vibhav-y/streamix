...
/*
 * sidebarFilter values:
 *   'home'     → popular videos (all categories)
 *   'shorts'   → search "#shorts"
 *   'trending' → popular/trending videos
 *   'music'    → category 10 (Music)
 */
...
export default function Home({ searchQuery, sidebarFilter = 'home' }) {
  ...
  /* ── Fetch videos based on search, sidebar filter, or category */
  const loadVideos = useCallback(async () => {
    ...
    try {
      let result
      if (searchQuery?.trim()) {
        // User typed a search — takes priority
        ...
        result = await fetchPopularVideos(20, '10') // Music category
      } else if (sidebarFilter === 'trending') {
        result = await fetchPopularVideos(20, '0')  // Trending = all popular
      } else {
  ...
  // Show category chips only on "home" filter with no search
  const showChips = sidebarFilter === 'home' && !searchQuery?.trim()

  return (
    <div className="p-6">
      {/* ── Section heading for sidebar filters ───────────── */}
      ...
      {/* ── Category filter chips (only on Home) ─────────── */}
      {showChips && (
